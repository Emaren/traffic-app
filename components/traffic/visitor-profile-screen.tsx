      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300">
                {profile.range_label} profile for this fingerprint. Traffic keeps the raw
                fingerprint exact, can still live-stream current movement when the visitor is
                active, and now surfaces likely linked fingerprints separately when the same source
                appears to shift browser identity.
              </p>
              {leadBadge ? (
                <div
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs font-medium text-emerald-200">
                  {profile.coverage_mode === "session_archive" ? "Session archive" : profile.coverage_mode === "durable_store" ? "Durable history" : "Live log fallback"}
                </span>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            helper={`Sessions tied to this exact visitor fingerprint in this ${sessionCoverageLabel}.`}
          />
          <StatCard
            label="Projects"
          ...
          <StatCard
            ...
            value={String(profile.visitor.linked_profiles_count)}
            helper="Neighboring fingerprints on the same source that Traffic thinks are likely related, without collapsing the raw history."
          />
          <StatCard
            label="Status"
        ...
        {profile.linked_profiles.length > 0 ? (
          <section className="mt-6 rounded-3xl border border-sky-400/20 bg-sky-400/5 p-5">
            <p className="text-xs uppercase tracking-[0.24em] text-slate-400">Identity hints</p>
            <h2 className="mt-2 text-2xl font-semibold text-white">Likely linked fingerprints</h2>
            <p className="mt-2 text-sm text-slate-300">
              Traffic is keeping raw fingerprints separate, but these nearby profiles share the same
              source context strongly enough to surface as likely related.
            </p>

            <div className="mt-5 grid gap-3 xl:grid-cols-2">
              {profile.linked_profiles.map((linked) => (
                <div
                  key={linked.id}
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <div className="break-words font-medium text-white">
                        {withFlag(linked.country_code, linked.alias)}
                      </div>
                      <div className="mt-1 text-xs text-slate-400">
                        {linked.device} • {linked.os} • {linked.browser}
                      </div>
                    </div>
                    <Link
                      href={`/visitors/${linked.id}`}
                  ...
                  <div className="mt-3 text-xs leading-6 text-slate-300">
                    <div>Reason: {linked.reason}</div>
                    <div>First seen: {linked.first_seen_alberta}</div>
                    <div>Last seen: {linked.last_seen_alberta}</div>
                    <div>
                      Sessions: {linked.total_sessions} • Projects: {linked.projects_visited}
                    </div>
                    <div className="break-words text-slate-400">
                      Projects: {linked.project_names.join(", ")}
                    </div>
                  </div>
                </div>
              ))}
              ...
              {visibleProjects.map((project) => (
                ...
                >
                  ...
                  <div className="mt-2 space-y-1 text-xs text-slate-400">
                    <div>First seen {project.first_seen_alberta}</div>
                    <div>Last seen {project.last_seen_alberta}</div>
                  </div>
                  <div className="mt-3">
                    <Link
